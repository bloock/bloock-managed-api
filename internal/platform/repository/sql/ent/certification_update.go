...
func (cu *CertificationUpdate) SetHash(s string) *CertificationUpdate {
...
func (cu *CertificationUpdate) check() error {
	if v, ok := cu.mutation.AnchorID(); ok {
...
func (cuo *CertificationUpdateOne) check() error {
	if v, ok := cuo.mutation.AnchorID(); ok {
		if err := certification.AnchorIDValidator(v); err != nil {
			return &ValidationError{Name: "anchor_id", err: fmt.Errorf(`ent: validator failed for field "Certification.anchor_id": %w`, err)}
		}
	}
	if v, ok := cuo.mutation.Hash(); ok {
		if err := certification.HashValidator(v); err != nil {
			return &ValidationError{Name: "hash", err: fmt.Errorf(`ent: validator failed for field "Certification.hash": %w`, err)}
		}
	}
